 ...
 */
@Component({
    selector: 'page-addon_datafield_consent_consent',
...
export default class AddonModDataEditPage implements OnInit {
    ...
    protected consentFieldsArray: AddonDatafieldConsentField[] = [];
    ...
    protected activeConsentFields: Record<number, number> = {};
    ...
    async ngOnInit(): Promise<void> {
        try {
            ...
            this.entryId = CoreNavigator.getRequiredRouteNumberParam('recordId');
            this.fieldId = CoreNavigator.getRequiredRouteNumberParam('fieldId');
    ...
    async canLeave(): Promise<boolean> {
        ...
        changed = changed || (this.initialSelectedGroup != this.selectedGroup);
    ...
    protected async fetchEntryData(refresh = false): Promise<void> {
        try {
            ...
            const activeField = this.fields[this.fieldId];
            // check field is of type consent
            if ( activeField.type != 'consent' ){
                return;
            }

            // check user matches requested user in consent record.
            const activeConsentRecord = this.originalData[this.fieldId];
            const consentItems = this.fieldsArray.filter(item => item.type === 'consent');

            this.consentFieldsArray = consentItems.map(f => {
                const relatedFields = (f.param3 ?? '')
                    .split(',')
                    .map(s => s.trim())
                    .filter(s => s.length > 0)
                    .map(s => this.fields[s]);
                ...
                return {
                    ...f,
                    relatedFields,   // <-- array of numbers
                };
            });

            if (activeConsentRecord === undefined) {
                // no content found for record.
                return;
            }
            const consentUserId = +activeConsentRecord.content;
            const currentUserId = CoreSites.getCurrentSiteUserId();
            if ( consentUserId != currentUserId) {
                return;
            }

            this.hasConsented = activeConsentRecord.content1 != null;
    ...
    protected displayEditFields(): string {
        ...
        this.consentFieldsArray.forEach((field) => {
            // Replace field by a generic directive.

            const isConsentFieldForCurrentUser = field.id === this.fieldId;
            const mode = !this.hasConsented && isConsentFieldForCurrentUser ? 'edit' : 'show';
            const color = isConsentFieldForCurrentUser ? 'secondary' : 'light';
            const you =  isConsentFieldForCurrentUser ? '(You)' : '';
            render += `<ion-accordion value="${field.id}">
                        <ion-item slot="header" color="${color}">
                            <ion-label>${field.name} Details  ${you}</ion-label>
                        </ion-item>
                        <div class="ion-padding bg-${color}" slot="content">`;
            field.relatedFields.forEach((relatedfield) => {
                render +=  `
                <h2>${relatedfield.name}</h2>
                <p>${relatedfield.description}</p>
                <addon-mod-data-field-plugin [class.has-errors]="!!errors[${relatedfield.id}]" mode="${mode}" \
                    [field]="fields[${relatedfield.id}]" [value]="contents[${relatedfield.id}]"
                    [form]="form" [database]="database" \
                    [error]="errors[${relatedfield.id}]" [recordHasOffline]="${this.entry?.hasOffline ? 'true' : 'false'}" \
                    (onFieldInit)="onFieldInit($event)"></addon-mod-data-field-plugin>`;
            });

            render +=  `
                <h2>${field.name} ${you}</h2>
                <p>${field.description}</p>
                <addon-mod-data-field-plugin [class.has-errors]="!!errors[${field.id}]" mode="show" \
...
type AddonDatafieldConsentField = AddonModDataField & {
    relatedFields: AddonModDataField[];
};
